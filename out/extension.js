...
function activate(context) {
    ...
    const disposables = [
        ...
        vscode.commands.registerCommand("serverRunner.retryServer", (serverId) => {
            const serverConfig = configManager.getServerById(serverId);
            if (serverConfig) {
    ...
    setInterval(() => {
        ...
        servers.forEach((server) => {
            ...
            if (terminal.exitStatus !== undefined) {
                ...
                console.log(`🔍 Terminal for ${server.name} has exited with status:`, terminal.exitStatus);
                return;
            }
...
function startServer(name, command, terminalKey) {
    ...
    const healthCheck = setInterval(() => {
        ...
        if (healthCheckAttempts >= 3) {
            ...
            if (!hasShownOutput) {
                vscode.window.showInformationMessage(`✅ ${name} started successfully!`);
                hasShownOutput = true;
            }
            return;
        }
        ...
        if (healthCheckAttempts >= maxHealthCheckAttempts) {
            ...
            if (terminals[terminalKey] &&
                ...
                serverProvider.updateServerStatus(terminalKey, "running");
                if (!hasShownOutput) {
                    vscode.window.showInformationMessage(`✅ ${name} appears to be running`);
                    hasShownOutput = true;
                }
            }
            else {
...
async function autoDetectProjects() {
    ...
    const selectedProjects = await showProjectSelectionDialog(detectedProjects, currentServerPaths, currentServerNames, currentServerNamesLowerCase);
...
function formatPathForTerminal(relativePath) {
    ...
    if (formattedPath.includes(' ') || formattedPath.includes('&') ||
        formattedPath.includes('(') || formattedPath.includes(')') ||
        formattedPath.includes("'") || formattedPath.includes('"')) {
...
function fixPathsInCommand(command) {
    ...
    return command.replace(/cd\s+([^\s&|]+)/g, (match, path) => {
        ...
        // Quote if necessary
        if (fixedPath.includes(' ') || fixedPath.includes('&') ||
            fixedPath.includes('(') || fixedPath.includes(')')) {
            return `cd "${fixedPath}"`;
        }
        return `cd ${fixedPath}`;
    });
...
async function checkForUpdates(context) {
    ...
    try {
        ...
        if (installedVersion !== lastNotifiedVersion && lastNotifiedVersion !== '0.0.0') {
        ...
        if (lastNotifiedVersion === '0.0.0') {
            setTimeout(() => {
                vscode.window.showInformationMessage('🥷 Welcome to Ninja Runner! Click the Ninja icon in the sidebar to get started.', 'Open Ninja Runner', 'View Documentation').then((selection) => {
...
function showReloadPrompt() {
    vscode.window.showInformationMessage('🔄 Ninja Runner has been updated! Please reload VS Code to use the latest features.', 'Reload Now', 'Later').then((selection) => {
